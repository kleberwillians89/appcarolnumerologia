...
export const renderNumerosPageB = async (
  pdf: jsPDF,
  numeros: NumeroItem[],
  nome: string,
  ...
  // Renderizar Sonho (índice 4), Dom (índice 1), Desafio Maior (índice 5)
  const indices = [4, 1, 5]; // Sonho, Dom, Desafio Maior
  
  for (let i = 0; i < indices.length; i++) {
    const numero = numeros[indices[i]];
    if (!numero) continue;
